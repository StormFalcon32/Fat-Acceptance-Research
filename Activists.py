...
def count():
    ...
    df = pd.read_csv(path / 'UserAndDates.csv', encoding='utf-8')
    num_general_tweets = 0
    num_activist_tweets = 0
    for i, row in df.iterrows():
        print(i)
        if 'retweeted_status' in json_data[str(row['num'])]:
            continue
        if row['num_tweets'] >= 10:
            num_activist_tweets += 1
        else:
            num_general_tweets += 1
    print('General: %s and Activist: %s' % (num_general_tweets, num_activist_tweets))
